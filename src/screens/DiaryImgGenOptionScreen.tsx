...
interface DiaryImgGenOptionScreenProps {
  navigation: any;
  route: {
    params: {
      diaryTitle: string;
      diaryContent: string;
      paragraphs: Paragraph[]; // 문단 데이터 추가
...
// ParagraphImageGeneration 인터페이스 제거 - paragraphs 상태로 대체
...
const DiaryImgGenOptionScreen: React.FC<DiaryImgGenOptionScreenProps> = ({ 
  navigation, 
  route 
}) => {
  const insets = useSafeAreaInsets();
  const { diaryTitle, diaryContent, paragraphs: initialParagraphs, selectedDate } = route.params;
  ...
  const [selectedStyle, setSelectedStyle] = useState<string>('children_book');
...
// paragraphGeneration 상태 제거 - paragraphs 상태로 대체
  ...
  const handleSavePress = () => {
    ...
    navigation.navigate('DiarySaveScreen', saveData);
  };

  const generateKeywordsFromDiary = async () => {
  ...
  const extractRecommendedSettings = () => {
    ...
    // 첫 번째 문단에서 추천 설정 추출 (모든 문단이 동일한 설정을 사용한다고 가정)
    const firstParagraph = paragraphs[0];
    if (!firstParagraph) return;
    
    // 문단의 키워드나 내용을 분석하여 추천 설정 생성
    // 실제로는 AI가 추천한 설정을 사용해야 하지만, 여기서는 기본값 사용
    const recommended = {
      style: 'children_book',
      mood: 'happy',
    ...
    console.log('🎨 [추천 설정] 추출된 이미지 설정:', recommended);
  };

  // processDiaryIntoParagraphs 함수 제거 - 이미 DiaryCreateScreen에서 처리됨
  ...
  const generateImageForParagraph = async (paragraphIndex: number) => {
    ...
    try {
      ...
      const styleConfig = { 
        type: (selectedStyleObj?.name || 'children_book') as any, 
        mood: (selectedMoodObj?.name || 'happy') as any, 
        colorPalette: (selectedColorPaletteObj?.name || 'vibrant') as any 
      };
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      if (generationMode === 'single') {
        ...
        // 장면 프롬프트를 이미지 설명으로 사용 (키워드는 사용하지 않음)
        ...
        generatedKeywords = []; // 단일 이미지 모드에서는 키워드 사용하지 않음
        ...
        imageDescription = `${diaryContent}의 내용을 바탕으로 한 아름다운 이미지`;
      }

      // 이미지 생성 요청 - 간소화된 설정 사용
      const selectedStyleObj = imageStyles.find(style => style.id === selectedStyle);
      const selectedMoodObj = imageMoods.find(mood => mood.id === selectedMood);
      const selectedColorPaletteObj = colorPalettes.find(palette => palette.id === selectedColorPalette);
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            {paragraphs.length === 0 ? (
              <View style={styles.paragraphProcessSection}>
                <Text style={styles.paragraphProcessText}>
                  문단 데이터가 없습니다. 일기 작성 화면에서 다시 시도해주세요.
                </Text>
              </View>
            ) : (
              <View style={styles.paragraphsContainer}>
                ...
                <View style={styles.imageSettingsContainer}>
                  ...
                  <ImageStyleSelector
                    ...
                    onCustomStyleToggle={handleCustomStyleToggle}
                    onCustomStyleAdd={handleCustomStyleAdd}
                    styles={styles}
        ...
        {generationMode === 'single' && (
          <>
                  ...
                  >
                    ...
                    <View style={styles.userPromptSection}>
                      ...
                      <TextInput
                        ...
                        placeholder="이미지에 추가하고 싶은 요소를 입력하세요 (예: 따뜻한 햇살, 꽃이 핀 배경 등)"
            ...
            {/* 단일 이미지 모드에서는 키워드 섹션 제거 - 선택된 문단의 장면 프롬프트만 사용 */}
...
const styles = StyleSheet.create({
  ...
  customStyleToggleActive: {
    backgroundColor: '#E3F2FD',
    paddingHorizontal: 12,
    paddingVertical: 8,
    borderRadius: 8,
  },
  customStyleToggleText: {
    fontSize: 14,
    color: '#666666',
    marginLeft: 8,
  },
  customStyleToggleTextActive: {
    color: '#1976D2',
    fontWeight: '600',
  },
  customStyleInput: {
  ...
  customStyleToggleDisabled: {
    opacity: 0.5,
  },
  customStyleToggleTextDisabled: {
    color: '#999999',
  },
  customStyleInputDisabled: {
    backgroundColor: '#F0F0F0',
    color: '#999999',
  },
  // 문단별 이미지 생성 스타일
...
export default DiaryImgGenOptionScreen;
